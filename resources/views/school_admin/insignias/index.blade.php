                ...
                <div class="card-body pt-0">
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                        ...
                        @foreach($insignias as $insignia)
                            ...
                            <tr>
                                <td>{{ $insignia->name }}</td>
                                <td>{{ $insignia->points_value }}</td>
                                <td><img src="{{ $insignia->image_url }}" alt="{{ $insignia->name }}" width="50px" height="50px"></td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center flex-shrink-0">
                                        <a href="javascript:void(0)"
                                           ...
                                           data-id="{{ $insignia->id }}"
                                           data-name="{{ $insignia->name }}"
                                           data-points="{{ $insignia->points_value }}"
                                        >
                                        ...
                                        <!-- زر حذف -->
                                        <form method="POST" action="{{ route('insignias.destroy',$insignia) }}">
                                            @csrf
                                            @method('DELETE')
                ...
                <form id="editCardForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body py-10 px-lg-17">
                        <div class="mb-5 fv-row">
                            <label class="required fs-5 fw-bold mb-2">اسم الشاره</label>
                            <input type="text" class="form-control form-control-solid" name="name"/>
                        </div>
                        <div class="mb-5 fv-row">
                            <label class="required fs-5 fw-bold mb-2">قيمة النقاط</label>
                            <input type="number" min="1" class="form-control form-control-solid" name="points_value"/>
                        </div>
                        <div class="mb-5 fv-row">
                            <label class="required fs-5 fw-bold mb-2">صورة الشارة</label>
...
@push("script")
    <script>
        ...
        $('.edit-btn').on('click', function() {
            ...
            form.attr('action', '/insignias/' + id);
            form.find('input[name="name"]').val(name);
            form.find('input[name="points_value"]').val(points);
