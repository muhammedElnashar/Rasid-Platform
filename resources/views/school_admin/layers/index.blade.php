                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                            <a href="{{ route('categories.layers.create',$category) }}" class="btn btn-primary" >
                                إضافة طبقة</a>
                        </div>
                    </div>
                </div>
                <div class="card-body pt-0">
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                        ...
                        @foreach($layers as $layer)
                            ...
                            <tr>
                                <td>{{ $layer->name }}</td>
                                <td>{{ $layer->reward_value }}</td>

                                <td>
                                    <div class="d-flex justify-content-center flex-shrink-0">
                                        <a href="{{route('categories.layers.levels.index',[$category,$layer])}}" class="btn btn-bg-light btn-active-color-primary btn-sm ms-2">المستويات</a>
                                        <a href="javascript:void(0)"
                                           class="btn btn-icon btn-bg-light edit-btn btn-active-color-primary btn-sm ms-2"
                                           data-id="{{ $layer->id }}"
                                           data-category_id="{{ $category->id }}"
                                           data-name="{{ $layer->name }}"
                                           data-reward_value="{{ $layer->reward_value }}"
                                        >
                                        ...
                                        <!-- زر حذف -->
                                        <form method="POST" action="{{ route('categories.layers.destroy',[$category,$layer]) }}">
                                            @csrf
                                            @method('DELETE')
...
@push("script")
    <script>
        ...
        $('.edit-btn').on('click', function() {
            ...
            form.attr('action', '/categories/' + categoryId + '/layers/' + id);
